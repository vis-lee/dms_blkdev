...
/********************************************************************************/
#if 0
static char check_error_case(struct request *req, struct request_queue *r_queue) {

//	if (!IS_LEGAL(DMSDEV_MOD, r_queue)){
//		eprintk("%s FATAL ERROR! ptr invalidate!\n", DMSDEV_MOD);
//		return -1;
//	}
...
//	if(elv_queue_empty(r_queue)){
//		DMS_PRINTK(DMSDEV_DBG, "%s%s elv_queue_empty\n", );
//		return 1;
//	}

//	if(list_empty(&req->queuelist)){
//		DMS_PRINTK(DMSDEV_DBG, "%s%s req->queuelist is empty\n", );
//		return -1;
//	}
...
//			geo.sectors = 16;
//			geo.start = 4;
//			if (copy_to_user((void *) ioctl_param, &geo, sizeof(geo)))
//				return -EFAULT;
//
//			return 0;
